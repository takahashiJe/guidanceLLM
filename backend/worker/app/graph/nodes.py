...
def agent_node(state: GraphState) -> dict:
    """RAGで取得したコンテキストをメッセージに含めてエージェントを起動する。"""
    messages_with_context = list(state["messages"])
    if state.get("context_documents"):
        context = "\n\n---\n\n".join([doc['content'] for doc in state["context_documents"]])
        system_context_message = SystemMessage(
            content=f"以下の参考情報を基に、ユーザーの質問に答えてください。\n\n# 参考情報\n{context}"
        )
        messages_with_context.insert(0, system_context_message)
    
    result = agent_executor.invoke({"messages": messages_with_context})
    return {"messages": [AIMessage(content=result["output"], tool_calls=result.get("tool_calls", []))]}
...
def generate_simple_response_node(state: GraphState) -> dict:
    ...
    simple_prompt = ChatPromptTemplate.from_messages([
        ("system", "あなたは親しみやすいアシスタントです。ユーザーと簡単な会話をしてください。"),
        MessagesPlaceholder(variable_name="messages")
    ])
